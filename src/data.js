const data = [
  {

    question: "Where was the BRICS summit held in 2014?",
    answers: [
      {
        choice: "A",
        text: "Brazil",
        correct: true,
      },
      {
        choice: "B",
        text: "India",
        correct: false,
      },
      {
        choice: "C",
        text: "Russia",
        correct: false,
      },
      {
        choice: "D",
        text: "China",
        correct: false,
      },
    ],
  },
  {
    question: "Which of these spices is the smallest in size?",
    answers: [
      {
        choice: "A",
        text: "Ajwain",
        correct: true,
      },
      {
        choice: "B",
        text: "Jeera",
        correct: false,
      },
      {
        choice: "C",
        text: "Saunf",
        correct: false,
      },
      {
        choice: "D",
        text: "Methi Seeds",
        correct: false,
      },
    ],
  },
  {
    question:
      "Which battle in 1757 marked the beginning of British occupation in India?",
    answers: [
      {
        choice: "A",
        text: "Plassey",
        correct: true,
      },
      {
        choice: "B",
        text: "Assaye",
        correct: false,
      },
      {
        choice: "C",
        text: "Buxar",
        correct: false,
      },
      {
        choice: "D",
        text: "Cuddalore",
        correct: false,
      },
    ],
  },
  {
    question: "Which is the second most spoken language of Nepal?",
    answers: [
      {
        choice: "A",
        text: "Bajjika",
        correct: false,
      },
      {
        choice: "B",
        text: "Nepali",
        correct: false,
      },
      {
        choice: "C",
        text: "Maithili",
        correct: true,
      },
      {
        choice: "D",
        text: "Bhojpuri",
        correct: false,
      },
    ],
  },
  {
    question: "In which of these two sports is the term ‘free hit’ used?",
    answers: [
      {
        choice: "A",
        text: "Football, Squash",
        correct: false,
      },
      {
        choice: "B",
        text: "Badminton, Tennis",
        correct: false,
      },
      {
        choice: "C",
        text: "Badminton, Cricket",
        correct: true,
      },
      {
        choice: "D",
        text: "Hockey, Cricket",
        correct: false,
      },
    ],
  },

  {
    question: "The earth is approximately how many miles away from the Sun?",
    answers: [
      {
        choice: "A",
        text: "9.3 million",
        correct: false,
      },
      {
        choice: "B",
        text: "39 million",
        correct: false,
      },
      {
        choice: "C",
        text: "93 million",
        correct: true,
      },
      {
        choice: "D",
        text: "193 million",
        correct: false,
      },
    ],

  },
  {
    question: "Which of the following men does not have a chemical element named after him?",
    answers: [
      {
        choice: "A",
        text: "Albert Einstein",
        correct: false,
      },
      {
        choice: "B",
        text: "Isaac Newton",
        correct: true,
      },
      {
        choice: "C",
        text: "Enrico Fermi",
        correct: false,
      },
      {
        choice: "D",
        text: "Enrico Fermi",
        correct: false,
      },
    ],

  },
  {
    question: "In the children's book series, where is Paddington Bear originally from?",
    answers: [
      {
        choice: "A",
        text: "India",
        correct: false,
      },
      {
        choice: "B",
        text: "Canada",
        correct: false,
      },
      {
        choice: "C",
        text: "Peru",
        correct: false,
      },
      {
        choice: "D",
        text: "Iceland",
        correct: true,
      },
    ],

  },
  {
    question: "Who did artist Grant Wood use as the model for the farmer in his classic painting American Gothic ?",
    answers: [
      {
        choice: "A",
        text: "His Denist",
        correct: true,
      },
      {
        choice: "B",
        text: "Travelling salesman",
        correct: false,
      },
      {
        choice: "C",
        text: "His butcher",
        correct: false,
      },
      {
        choice: "D",
        text: "Local sheriff",
        correct: false,
      },
    ],

  },
  {
    question: "Now used to refer to a cat, the word tabby is derived from the name of a district of what world capital?",
    answers: [
      {
        choice: "A",
        text: "Baghdad",
        correct: true,
      },
      {
        choice: "B",
        text: "New Delhi",
        correct: false,
      },
      {
        choice: "C",
        text: "Moscow",
        correct: false,
      },
      {
        choice: "D",
        text: "Cairo",
        correct: false,
      },
    ],

  },
  {
    question: "If you planted the seeds of Quercus robur, what would grow?",
    answers: [
      {
        choice: "A",
        text: "Flowers",
        correct: false,
      },
      {
        choice: "B",
        text: "Grain",
        correct: false,
      },
      {
        choice: "C",
        text: "Trees",
        correct: true,
      },
      {
        choice: "D",
        text: "Vegetables",
        correct: false,
      },
    ],

  },
  {
    question: "What club did astronaut Alan Shepard use to make his famous golf shot on the moon?",
    answers: [
      {
        choice: "A",
        text: "Nine iron",
        correct: false,
      },
      {
        choice: "B",
        text: "Sand wedge",
        correct: false,
      },
      {
        choice: "C",
        text: "Seven iron",
        correct: false,
      },
      {
        choice: "D",
        text: "Six iron",
        correct: true,
      },
    ],

  },
  {

    question: "Neurologists believe that the brain's medial ventral prefrontal cortex is activated when you do what?",
    answers: [
      {
        choice: "A",
        text: "Get a joke",
        correct: true,
      },
      {
        choice: "B",
        text: "Have a panic attack",
        correct: false,
      },
      {
        choice: "C",
        text: "Remember a name",
        correct: false,
      },
      {
        choice: "D",
        text: "Listen to music",
        correct: false,
      },
    ],

  },
  {

    question: "Which king was married to Eleanor of Aquitaine?",
    answers: [
      {
        choice: "A",
        text: "Henry I",
        correct: false,
      },
      {
        choice: "B",
        text: "Henry II",
        correct: true,
      },
      {
        choice: "C",
        text: "Richard I",
        correct: false,
      },
      {
        choice: "D",
        text: "Henry V",
        correct: false,
      },
    ],

  },
  {

    question: "And which of these is not one of the American Triple Crown horse races?",
    answers: [
      {
        choice: "A",
        text: "Arlington Million",
        correct: true,
      },
      {
        choice: "B",
        text: "Preakness Stakes",
        correct: false,
      },
      {
        choice: "C",
        text: "Belmont Stakes",
        correct: false,
      },
      {
        choice: "D",
        text: "Kentucky Derby",
        correct: false,
      },
    ],
  },
].sort(() => Math.random() - 0.5)

const prizeMoney = [
  { id: 1, amount: "$ 100" },
  { id: 2, amount: "$ 200" },
  { id: 3, amount: "$ 300" },
  { id: 4, amount: "$ 500" },
  { id: 5, amount: "$ 1000" },
  { id: 6, amount: "$ 2000" },
  { id: 7, amount: "$ 4000" },
  { id: 8, amount: "$ 8000" },
  { id: 9, amount: "$ 16000" },
  { id: 10, amount: "$ 32000" },
  { id: 11, amount: "$ 64000" },
  { id: 12, amount: "$ 125000" },
  { id: 13, amount: "$ 250000" },
  { id: 14, amount: "$ 500000" },
  { id: 15, amount: "$ 1000000" },
].reverse();

export { prizeMoney, data };
